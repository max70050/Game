...
#pauseButton svg {
  width: 24px;
  height: 24px;
  fill: white; /* Weiß, damit es zum UI passt */
}

#pauseButton:hover svg {
  fill: lightgray; /* Hover-Effekt */
}
        #highscore {
      display: inline-block;
...
<button id="pauseButton" title="Pause / Resume">
...
document.getElementById("pauseButton").addEventListener("click", function () {
  ...
  if (isPaused) {
    pauseGame();
    pauseIcon.style.display = "none"; // Pause-Symbol ausblenden
    playIcon.style.display = "block"; // Play-Symbol einblenden
  } else {
    resumeGame();
    pauseIcon.style.display = "block"; // Pause-Symbol wieder anzeigen
    playIcon.style.display = "none"; // Play-Symbol ausblenden
  }
});

function pauseGame() {
  console.log("Spiel pausiert");
  running = false; // Setze die Variable "running" auf false, um das Spiel zu pausieren
}

function resumeGame() {
  console.log("Spiel fortgesetzt");
  running = true; // Setze die Variable "running" auf true, um das Spiel fortzusetzen
  requestAnimationFrame(update); // Starte die Animation neu
}
